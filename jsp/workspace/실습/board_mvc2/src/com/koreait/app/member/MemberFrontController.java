...
public class MemberFrontController extends HttpServlet {
	 ...
	 * 사용한다. 직렬화 된 객체는 byte형태로 변환되어 있으며, 다시 사용하고 싶을 때에는 역직렬화를 통해서 객체로 변환된다. 이 때
	...
	protected void doProcess(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (command.equals("/member/MemberJoinOk.me")) {
			...
			forward = new ActionForward();
			forward.setRedirect(false);
			// ()로 묶지 않으면 연산자 우선순위 때문에 "/app/member/login.jsp"+ login 이게 먼저 적용되버린다.
			// 만약 login에 false가 들어있다면 .jsp 파일에도 전달해준다.
